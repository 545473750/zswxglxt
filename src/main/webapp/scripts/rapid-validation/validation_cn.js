...
Validator.messageSource['en-us'] = [
	...
	['validate-unfile' , function(v,elm,args,metadata) {
		return ValidationUtils.format("Please enter file type not in [%s]",[args.join(',')]);
	}],
	//中国特有的相关验证提示信息
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-decimalPlaces' , '精确到小数点后%s位'],
	['validate-integerBits' , '整数位不能超过%s位'],
	...
	['validate-unfile' , function(v,elm,args,metadata) {
		return ValidationUtils.format("文件类型应该为除[%s]以外的其他类型",[args.join(',')]);
	}],
	
	//中国特有的相关验证提示信息
	['validate-id-number','请输入合法的身份证号码'],
	['validate-chinese','请输入中文'],
	['validate-phone','请输入正确的电话号码或者手机号'],
	['validate-mobile-phone','请输入正确的手机号码,当前长度为%s.'],
	['validate-zip','请输入有效的邮政编码'],
	['validate-qq','请输入有效的QQ号码.'],
	['validate-uri' , '请输入有效的网址.'],
	['validate-card-equals-birthday','生日和身份证上的不匹配']
]
...
Validation.addAllThese([
	...
	['validate-uri',/^(([A-Z0-9][A-Z0-9_-]*)(\.[A-Z0-9][A-Z0-9_-]*)+)(:(\d+))?\/?/i],
	// [$]1[##][,###]+[.##]
	// [$]1###+[.##]
	// [$]0.##
	// [$].##
...
Validation.addAllThese([
	...
	/*
	 * Usage: validate-unfile-$type1-$type2-$typeX
	 * Example: validate-unfile-png-jpg-jpeg
	 */
	['validate-unfile',function(v,elm,args,metadata) {
		return $A(args).all(function(extentionName) {
			var index = v.lastIndexOf(".");
			if(index!=-1){
				var fix = v.substring(index+1);
				var validate = new RegExp('^'+extentionName+'$','i').test(fix);
				return !validate;
			}
			return true;
		});
	...
	['validate-ajax',function(v,elm,args,metadata) {
		...
		/*//alert(params);
		var x = params.indexOf("&photoFile");
		var y = params.indexOf("&",x+1);
		//alert(x);
		//alert(y);
		var paramsA = params.substring(0,x);
		//alert(paramsA);
		var paramsB = params.substring(y,params.lenght);
		//alert(paramsB);
	...
	['validate-phone',/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/
	...
	/*
	 * Usage: validate-decimalPlaces-$integer 验证浮点型“小数位数”
	 * Example: <input id='money' class='validate-decimalPlaces-2'>
	 */
	['validate-decimalPlaces',function(v,elm,args,metadata){
		var reg = '/^-?[0-9]+\.?[0-9]{0,'+args.singleArgument+'}$/';
		return eval('('+reg+'.test(v))');
	}],
	/*
	 * Usage: validate-integerBits-$integer 验证浮点型“整数位”
	 * Example: <input id='money' class='validate-integerBits-2'>
	 */
	['validate-integerBits',function(v,elm,args,metadata){
		if(v.indexOf("\.")==-1){
			return v.length <= parseInt(args[0]);
		}
		var reg = '/^[0-9]{0,'+args.singleArgument+'}\.?[0-9]*$/';
		return eval('('+reg+'.test(v))');
	}],

	['validate-card-equals-birthday',function(v,elm,args,metadata){
		var cardId = $F(args[0]);
