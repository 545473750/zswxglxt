...
<script type="text/javascript">
		...
		function delItem(id, name){
			//删除列表项
			var userList = document.getElementById("selectedHTML");
		...
		function submitValue() {
			//var ids = document.getElementById("ids").value;
			//var names = document.getElementById("names").value;
			//window.parent.document.getElementById("userIds").value = ids;
			//window.parent.document.getElementById("userTexts").value = names;
		...
		function resetValue() {
			...
			document.getElementById('selectedHTML').innerHTML = "";
...
<div class="op_box_cen" style="width: 100%; overflow: hidden; height: 334px;">
	...
	<form method="post" action="${ctx }/organiz/User!configStation.do" id="submitForm">
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="gridBody">
	  <tr><td>
	  <!-- user id -->
	  <input type="hidden"" name="id" value="${id }"/>
	  <!-- ids -->
	  <input type="hidden" name="selectedIds" id="ids" value="${selectedIds }"/>
	  <input type="hidden" name="selectedNames" id="names" value="${selectedNames }"/>
	  
	  <input type="hidden" name="dialogId" value="stationDialog"/>
	  </td></tr>
	</table>
	</form>
	</div>
	...
	<iframe src="${pageContext.request.contextPath }/organiz/User!stationList.do" style="border:none;" frameborder="0" width="100%" height="206" scrolling="auto">
	</iframe>
	</div>

	<!-- 已选岗位列表 -->
	<div class="iframe_name" style="overflow: auto; height: 80px; align:center">
		<div id="selectedHTML">${selectedHTML }</div>
