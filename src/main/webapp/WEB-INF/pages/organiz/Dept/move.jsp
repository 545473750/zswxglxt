...
<script language="javascript">
	$(function() {
		...
		tree.setXMLAutoLoading("${ctx}/organiz/Organization!getOrganizationTreeXmlWithRoot.do?selectedIds=${parentOrganizationId}&partitionId=${partitionId}&id=${id}");
		tree.loadXML("${ctx}/organiz/Organization!getOrganizationTreeXmlWithRoot.do?selectedIds=${parentOrganizationId}&partitionId=${partitionId}&id=${id}");
...
<%-- 	${id} <br /> ${parentOrganizationId } <br /> --%>
	<form method="post" name="moveForm" id="moveForm" action="${ctx }/organiz/Organization!moveOperation.do">
		<input type="hidden" name="parentOrganizationId" id="moveToNodeId"/>
