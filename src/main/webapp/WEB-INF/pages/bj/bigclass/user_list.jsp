...
<body>
  ...
  <div class="llistbox">
    ...
    <div class="centent">
      <div class="search">
        <form id="queryForm" name="queryForm" method="post" action="${ctx}/bj/ClassBigclass!userlist.do">
			<input type="hidden" name="teacher" value="${teacher }"/>
        	<table>
        	<c:if test="${teacher != 'local'}">
				<tr>
					<td>姓名：</td>
...
<div class="table01">
<form id="listForm" name="listForm" method="post" action="${ctx}/bj/ClassBigclass!userlist.do">
<span>
...
<c:forEach var="obj" items="${page.result}" varStatus="vs">
	<tr class="${vs.count % 2 == 0 ? 'jg' : ''}" >
		<td align="center">
			...
			<input type="hidden" name="username2" value="${obj.user.username}"/>
		</td>
		<td align="center">
           ...
           <td width="70%">
            <div class="pages">
				<jsp:include flush="true" page="/commons/pagebar.jsp">
					<jsp:param name="target" value="/bj/ClassBigclass!userlist.do?teacher=${teacher}&username=${username }&orgName=${orgName }&userNum=${userNum }" />
		        </jsp:include>
		    </div>
		   </td>
		  </tr>
...
function adduser(checkboxName,form){
    ...
    for(var i = 0; i < items.length; i++){
    	if(items[i].checked == true){
    		userid = items[i].value;
    		username = username2[i].value;
    	}
    }
    parent.mainFrame.window.setIdAndName("${teacher}",username,userid);
