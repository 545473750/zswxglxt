...
<div class="llistbox">
	 <div class="listmap">
          <p class="dqwz">当前位置：<a href="#">教师培训</a> &gt; <a href="#">培训班管理</a> &gt; 修改培训班</p>
          <p class="fhbut">
            <input type="button" name="button" id="button" value="返 回" onclick="goBack()" />
          </p>
      </div>
    <div class="tablelist">
	<div class="title">修改培训班</div>
	<div class="table01">
  	<form id="dataform" name="dataform" method="post"  action="${ctx}/bj/ClassBigclass!saveEdit.do"  enctype="multipart/form-data">
...
<script type="text/javascript">
	...
	//打开dialog
	function openDialog(id,value,khvalue){
		...
		var khvalues = khvalue.split(",");
		var khKeys = $("input[name=khKey]");
		for(var j = 0; j < khvalues.length; j++){
			var khKeys = $("input[name=khKey]");
			for(var i = 0; i < khKeys.length; i++){
				var kh = $("input[name=khKey]").eq(i);
				if(kh.val() == khvalues[j]){
					kh.attr("checked",true);
				}
			}
		}
		$("#showKh").dialog("open");
	...
	function saveAndSend(action,form){
		...
		if(confirm("确定要提交 并发送审批？发送审批后将不能修改！")){
			form.action = action;
			$("#dataform").submit();
		}
	}
