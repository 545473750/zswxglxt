	...
	<tr class="${vs.count % 2 == 0 ? 'jg' : ''}" >
		...
		<td>
				<c:if test="${fn:length(obj.title)>10}">
					<a href="${ctx}/bj/ClassRecruit!detailed.do?id=${obj.id}&type=1&bmState=${bmState}" title="${obj.title }">
						${fn:substring(obj.title,0,10)}...
					</a>
				</c:if>
				<c:if test="${fn:length(obj.title)<=10}">
					<a href="${ctx}/bj/ClassRecruit!detailed.do?id=${obj.id}&type=1&bmState=${bmState}">${obj.title}</a>
				</c:if>
		</td>
		<td>
