...
<!-- Conter -->
<div id="content">
  <div class="dqwz">当前位置：<a href="http://yxwx.bjchyedu.cn/" > 首  页</a> &gt; <a href="#">活动列表</a> </div>
  <div class="show_box">
    <div class="list_title">活动</div>
    <div class="list_text2">
      <table style="width: 100%">
      <ul>
     <c:forEach var="activity" items="${page.result}" varStatus="vs">
     	 <li><span><fmt:formatDate value="${activity.ts}" pattern="yyyy-MM-dd"/></span><label title="${activity.name}"> 
               <c:set var="name" value="${fn:trim(activity.name)}" />
                <c:choose> 
	                <c:when test="${fn:length(activity.name)>13}"> 
	                	<a href="${ctx}/portal/Hd!view.do?id=${activity.id}"><c:out value="${fn:substring(activity.name,0,13)}..." /> </a>
	                </c:when> 
	                <c:otherwise> 
	                	<a href="${ctx}/portal/Hd!view.do?id=${activity.id}"><c:out value="${activity.name}" /></a>
	                </c:otherwise> 
                </c:choose> 
            </label></li>
         	</c:forEach></ul>
           ...
           <td width="70%">
            <div class="pages">
				<jsp:include flush="true" page="/commons/pagebar.jsp">
					<jsp:param name="target" value="/portal/Hd!list.do" />
		        </jsp:include>
        	</div>
           </td>
        </tr>
