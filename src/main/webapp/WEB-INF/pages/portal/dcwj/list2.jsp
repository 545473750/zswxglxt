...
<!-- Conter -->
<div id="content">
  <div class="dqwz">当前位置：<a href="http://yxwx.bjchyedu.cn/" > 首  页</a> &gt; <a href="#">研修成果列表 </a> </div>
  <div class="show_box">
    <div class="list_title">研修成果</div>
    <div class="list_text2">
       <table style="width: 100%">
             <ul>
     <c:forEach var="dcwj" items="${page.result}" varStatus="vs">
     	 <li><span><fmt:formatDate value="${dcwj.addtime}" pattern="yyyy-MM-dd"/></span><label title="${dcwj.spTitle}">
               <c:set var="spTitle" value="${fn:trim(dcwj.spTitle)}" />
                <c:choose> 
	                <c:when test="${fn:length(dcwj.spTitle)>14}"> 
	                	<a href="${ctx}/portal/Dcwj!view.do?spId=${dcwj.spId}"><c:out value="${fn:substring(dcwj.spTitle,0,14)}..." /> </a>
	                </c:when> 
	                <c:otherwise> 
	                	<a href="${ctx}/portal/Dcwj!view.do?spId=${dcwj.spId}"><c:out value="${dcwj.spTitle}" /></a>
	                </c:otherwise> 
                </c:choose> 
            </label></li>
         	</c:forEach></ul>
           ...
           <td width="70%">
            <div class="pages">
				<jsp:include flush="true" page="/commons/pagebar.jsp">
					<jsp:param name="target" value="/portal/Dcwj!list.do" />
		        </jsp:include>
        	</div>
           </td>
        </tr>
