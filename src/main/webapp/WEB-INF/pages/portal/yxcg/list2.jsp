...
<div id="content">
  ...
  <div class="show_box">
    ...
    <div class="list_text2">
     ...
     <ul>
     <c:forEach var="course" items="${page.result}" varStatus="vs">
     	 <li><span><fmt:formatDate value="${course.ts}" pattern="yyyy-MM-dd"/></span><label title="${course.name}">
                <c:set var="name" value="${fn:trim(course.name)}" />
                <c:choose> 
	                <c:when test="${fn:length(xdk.name)>14}"> 
	                	<a href="${ctx}/portal/Yxcg!view.do?id=${course.id}"><c:out value="${fn:substring(course.name,0,14)}..." /> </a>
	                </c:when> 
	                <c:otherwise> 
	                	<a href="${ctx}/portal/Yxcg!view.do?id=${course.id}"><c:out value="${course.name}" /></a>
	                </c:otherwise> 
                </c:choose> 
            </label></li>
         	</c:forEach></ul>
           ...
           <td width="70%">
            <div class="pages">
				<jsp:include flush="true" page="/commons/pagebar.jsp">
					<jsp:param name="target" value="/portal/Yxcg!list.do" />
		        </jsp:include>
        	</div>
           </td>
        </tr>
