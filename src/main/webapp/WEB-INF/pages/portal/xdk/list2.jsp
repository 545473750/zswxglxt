...
<!-- Conter -->
<div id="content">
  <div class="dqwz">当前位置：<a href="http://yxwx.bjchyedu.cn/" > 首  页</a> &gt; <a href="#">选单课列表 </a> </div>
  <div class="show_box">
     	 ...
     	 <li><span><fmt:formatDate value="${xdk.ts}" pattern="yyyy-MM-dd"/></span><label title="${xdk.name}">
                ...
                <c:choose> 
	                <c:when test="${fn:length(xdk.name)>14}"> 
	                	<a href="${ctx}/course/myCourse!choose.do?detailId=${xdk.id}"><c:out value="${fn:substring(xdk.name,0,14)}..." /> </a>
	                </c:when> 
	                <c:otherwise> 
	                	<a href="${ctx}/course/myCourse!choose.do?detailId=${xdk.id}"><c:out value="${xdk.name}" /></a>
	                </c:otherwise> 
                </c:choose> 
            </label></li>
         	</c:forEach></ul>
           ...
           <td width="70%">
            <div class="pages">
				<jsp:include flush="true" page="/commons/pagebar.jsp">
					<jsp:param name="target" value="/portal/Xdk!xdlist.do" />
		        </jsp:include>
        	</div>
           </td>
        </tr>
