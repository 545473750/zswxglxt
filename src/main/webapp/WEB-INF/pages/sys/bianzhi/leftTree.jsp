...
<head>
	...
	$(function() {
		...
		tree.setXMLAutoLoading("${ctx}/sys/Bianzhi!deptXML.do");
		tree.loadXML("${ctx}/sys/Bianzhi!deptXML.do");
...
//		window.parent.rightFrame.location.href = "${ctx}/sys/Bianzhi!list.do?query.orgId=" + id;
		//如果节点id等于当前单位id
		
		if('${orgId}'==id){
			window.parent.rightFrame.location.href = "${ctx}/sys/Bianzhi!list.do?query.orgId=${orgId}";
		}else{
			window.parent.rightFrame.location.href = "${ctx}/sys/Bianzhi!list.do?query.orgId=${orgId}&query.deptId=" + id;
		}
	}
	</script>
</head>
...
-->
<div id="divForTree" style="height: 400px;"></div>
</body>
</html>
