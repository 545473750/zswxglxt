...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <div class="maintit">
    ...
    <div class="centent">
      <div class="search">
        <form id="queryForm" name="queryForm" method="post" action="${ctx}/wf/WfActivityParticipan!orglist.do">
...
<div class="shop" id="shop">
        <p class="left">
          <input type="button" class="but_shop"  onclick="addorg('${ctx}/wf/WfActivityParticipan!addparticipant.do','items',document.listForm);" value="添加" />
        </p>
        <p class="right">
        	<input type="button" value="返回" class="but_shop" onclick="goBack()"/>
	          <select name="sortColumns" id="sortColumns" class="font12" onchange="sortColumns(this.value,document._pageForm_)" >
				...
				<jsp:param name="target" value="/wf/WfActivityParticipan!orglist.do" />
	           </jsp:include>
      		      	</p>
        <div class="clear"></div>
</div>
<form id="listForm" name="listForm" method="post" action="${ctx}/wf/WfActivityParticipan!orglist.do">
<span>
...
<c:forEach var="obj" items="${page.result}" varStatus="vs">
   ...
   <div class="page">
		<jsp:include flush="true" page="/commons/pagebar.jsp">
			<jsp:param name="target" value="/wf/WfActivityParticipan!orglist.do?activityId=${activityId}&proId=${proId}" />
        </jsp:include>
   </div>
  </div>
</div>
<script type="text/javascript">
function addorg(action,checkboxName,form){
    if (!hasOneChecked(checkboxName)){
            alert('请选择要操作的对象!');
            return;
    }
    var actId = document.getElementsByName("activityId")[0].value;
    var proId = document.getElementsByName("proId")[0].value; 
    form.action = action +"?query.wfActivity.id="+ actId +"&query.proId="+proId;
    form.submit();
}

function goBack(){
	location.href="${ctx }/wf/WfActivityParticipan!list.do?activityId=${activityId}&proId=${proId}&query.groupFlag=${groupFlag}";
}
	$("#shop").smartFloat();  
</script>
  </body>
